...
public class SeedingDatabase implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		maria = userRepository.searchByEmail(maria.getEmail()).toFuture().get();
		alex = userRepository.searchByEmail(alex.getEmail()).toFuture().get();
		bob = userRepository.searchByEmail(bob.getEmail()).toFuture().get();
		...
		post1.setUser(userRepository.searchByEmail("maria@gmail.com").block());
		post2.setUser(userRepository.searchByEmail("alex@gmail.com").block());
